...
router.get('/gis_agent', async function (req, res, next) {

    // res.json({success: true, message: objid_coord,geomtype:geomType});
 
 });

 router.put('/gis_agent', async function (req, res, next) {
...
router.post('/gis_agent', async function (req, res, next) {

      
    const customTool = new DynamicStructuredTool({
        name: "Geocode_Address",
        description: "Geocodes an address based on user input",
        schema: z.object({
            address: z.string().describe("The address to be geocoded "),
